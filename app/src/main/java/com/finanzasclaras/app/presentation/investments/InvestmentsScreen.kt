...
private fun InvestmentCard(investment: Investment, currency: String, onEdit: () -> Unit) {
    val dateStr = remember(investment.purchaseDate) {
        SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()).format(Date(investment.purchaseDate))
    }
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Row(
                ...
                Column(horizontalAlignment = Alignment.End) {
                    ...
                    Text(
                        text = "${String.format("%.2f", investment.profitLossPercentage)}%",
